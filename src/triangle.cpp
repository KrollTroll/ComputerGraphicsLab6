 ...
 */
triangle::triangle(float x, float y, int R, int G, int B, float x2, float y2, float x3, float y3):shape(x,y,R,G,B), coor2(3,1), coor3(3,1){
	this->x2 = x2;
	this->y2 = y2;
	this->x3 = x3;
	this->y3 = y3;
 ...
 */
triangle::triangle(const triangle& from):shape(from), coor2(3,1), coor3(3,1){
	this->x2 = from.x2;
	this->y2 = from.x2;
	this->x3 = from.x3;
	this->y3 = from.x3;
...
triangle& triangle::operator=(const triangle& from){
	...
	this->coor2[1][0] = this->y2 = from.y2;
	this->coor3[0][0] = this->x3 = from.x3;
	this->coor3[1][0] = this->y3 = from.y3;
	return *this;
}
...
void triangle::in(std::istream& in) {
		...
		//If the file is valid
			...
			string holder;
				...
				getline(in, holder);
				holder.erase(std::remove(holder.begin(),holder.end(),' '),holder.end());
				this->coor3[0][0] = this->x3 = stof(holder);
				getline(in, holder);
				holder.erase(std::remove(holder.begin(),holder.end(),' '),holder.end());
				this->coor3[1][0] = this->y3 = stof(holder);
		}
}
